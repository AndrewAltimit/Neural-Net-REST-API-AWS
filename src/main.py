...
def process_image_CNN(image_path):
	...
	# Resize the Image and convert to numpy array
	resized_image = image.resize((RES_WIDTH, RES_HEIGHT), Image.ANTIALIAS)
	resized_image = np.array(resized_image.getdata()).reshape(32,32,3)
...
def test(model, test_dataset, process_function, classes):
...
if __name__ == "__main__":
	...
	if mode == "FCNN":
		...
		if not LOAD_MODEL:
			model = train(training_dataset, Net, process_image_FCNN)
		
		# Evaluate the model with the testing data
		test(model, test_dataset, process_image_FCNN, test_folders)
		
	elif mode == "CNN":
		### CNN ###
		# Build the model with the training data
		if not LOAD_MODEL:
			model = train(training_dataset, ConvNet, process_image_CNN)
		
		# Evaluate the model with the testing data
		test(model, test_dataset, process_image_CNN, test_folders)
